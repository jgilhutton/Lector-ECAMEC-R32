class Serie:
    ...
    headerMap = ['fileName', 'equipo', 'periodo', 'dStrt', 'mStrt', 'yStrt', 'hStrt', 'minStrt', 'vNomRaw', 'iNomRaw', 'dEnd', 'mEnd', 'yEnd',
                 'hEnd', 'minEnd']
    # UNPACK STRINGS
...
class Serie12(Serie):
    ...
    def analizarRegistroDat(self, reg, calibr, tv, ti, header):
        reg.v1 = reg.v1Raw * (header.vNom / calibr.cV1) * tv
        reg.v1Max = reg.v1MaxRaw * (header.vNom / calibr.cV1) * tv
        reg.v1Min = reg.v1MinRaw * (header.vNom / calibr.cV1) * tv
        ...
        reg.v2 = reg.v2Raw * (header.vNom / calibr.cV2) * tv
        reg.v2Max = reg.v2MaxRaw * (header.vNom / calibr.cV2) * tv
        reg.v2Min = reg.v2MinRaw * (header.vNom / calibr.cV2) * tv
        ...
        reg.v3 = reg.v3Raw * (header.vNom / calibr.cV3) * tv
        reg.v3Max = reg.v3MaxRaw * (header.vNom / calibr.cV3) * tv
        reg.v3Min = reg.v3MinRaw * (header.vNom / calibr.cV3) * tv
...
class Serie15(Serie):
    ...
    headerMap = ['serie', 'equipo', 'periodo', 'dStrt', 'mStrt', 'yStrt', 'hStrt', 'minStrt', 'dEnd', 'mEnd', 'yEnd',
                 'hEnd', 'minEnd']
    reverse = True
...
class Serie0C(Serie):
    ...
    def analizarRegistroDat(self, reg, calibr, tv, ti, header):
        ...
        reg.e1p = ((((reg.e1b1 << 7) + reg.e1b2) << 7) + reg.e1b3) * self.eCoef / 1000 * tv * ti
        reg.e2p = ((((reg.e2b1 << 7) + reg.e2b2) << 7) + reg.e2b3) * self.eCoef / 1000 * tv * ti
        reg.e3p = ((((reg.e3b1 << 7) + reg.e3b2) << 7) + reg.e3b3) * self.eCoef / 1000 * tv * ti
        ...
        try:
            reg.phi1 = reg.e1p / (reg.i1p * reg.v1 / 1000 * header.periodo / 60)
        except ZeroDivisionError:
            reg.phi1 = 0.0
        try:
            reg.phi2 = reg.e2p / (reg.i2p * reg.v2 / 1000 * header.periodo / 60)
        except ZeroDivisionError:
            reg.phi2 = 0.0
        try:
            reg.phi3 = reg.e3p / (reg.i3p * reg.v3 / 1000 * header.periodo / 60)
        except ZeroDivisionError:
            reg.phi3 = 0.0

        reg.e1 = reg.v1*reg.i1*reg.phi1/1000*header.periodo/60
        reg.e2 = reg.v2*reg.i2*reg.phi2/1000*header.periodo/60
        reg.e3 = reg.v3*reg.i3*reg.phi3/1000*header.periodo/60
