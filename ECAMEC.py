...
0x02:'\x14',0x04:'\x16',0xA0:'\x17',0xAB:'\x18',0xAD:'\x19',0x5B:'\x0C',0xFF:'\x1A',0x82:'\x1B',0x05:'\x1C',0x84:'\x1D',0x45:'\x1E',
...
class Ecamec():
    ...
    def procesarR32(self,file):
        ...
        for medicion in extractor:
            header = medicion.pop()
            try: calibraciones = medicion.pop()
            ...
            for registro in medicion:
                if type(registro) == RegistroErr:
                    ...
                    registro = self.r32.tipoEquipo.analizarRegistroDat(self.r32.tipoEquipo,registro,calibraciones,self.TV,self.TI,header.periodo)
                    if corteEnProgreso:
    ...
    def genDat(self):
        ...
        header = (yield)
        headerStr = self.r32.tipoEquipo.headerFormatString.format(header.fileName,'-',header.serie.decode('utf-8') if hasattr(header,'serie') else 'ND',header.periodo,'min','220',self.TV,'5',self.TI,header.fechaInicio,header.fechaFin,header.horaInicio,header.horaFin)
        outputFile.write(headerStr)
        while True:
            reg = (yield)
            arguments = (reg.fecha,reg.hora)+tuple((getattr(reg,x) for x in self.r32.tipoEquipo.regDatMap))+(reg.anormalidad,)
    ...
    def extraerData(self):
        registros = []
        try:reverse = self.r32.tipoEquipo.reverse
        except: return
