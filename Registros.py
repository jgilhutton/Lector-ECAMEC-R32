...
class Header(Registro):
    def setData(self):
        try:self.fileName = self.fileName.strip(b'\x00').decode('utf-8')
        except AttributeError: pass
        self.periodo,self.dStrt,self.mStrt,self.yStrt,self.hStrt,self.minStrt,self.dEnd,self.mEnd,self.yEnd,self.hEnd,self.minEnd = (int(x.decode('utf-8')) for x in (self.periodo,self.dStrt,self.mStrt,self.yStrt,self.hStrt,self.minStrt,self.dEnd,self.mEnd,self.yEnd,self.hEnd,self.minEnd))
        self.timeStampStart = strptime(','.join((str(x) for x in (self.dStrt,self.mStrt,self.yStrt,self.hStrt,self.minStrt))),'%d,%m,%y,%H,%M')
        self.timeStampEnd = strptime(','.join((str(x) for x in (self.dEnd,self.mEnd,self.yEnd,self.hEnd,self.minEnd))),'%d,%m,%y,%H,%M')
...
class RegistroErr(Registro):
    def setData(self):
        ...
        self.timeStamp = strptime(','.join((str(x) for x in (self.d,self.m,self.y,self.H,self.M,self.S))),'%d,%m,%y,%H,%M,%S')
